...
\footnotetext{\textit{Triggers} are points in time when the matched-filter SNR is greater than a threshold value for a given gravitational wave template~\cite{pycbc_og1}.}
...
\av{this p-astro doesnt sit well with me... }
...
\caption[High-mass BCR search space.]{The template bank used by \pycbc to search a section of O2 data from $\text{April 23 - May 8, 2017}$. Our search is constrained to the high-mass parameter space enclosed by the dashed line. \av{This plot isnt adding anything... is it needed?}}\label{fig:templateBank}
\end{figure}
...
\av{talk about the correct time}
...
In addition to the above confirmed gravitational wave events from GWTC-1, we have also analysed several candidate events discussed in GWTC-1, most of which have low \pastrobcr. For example, consider the candidate event GWC170412, assigned a $\pastro$ of $0.06$ by \gstlal and has a $\pastrobcr$ of $0.01$. This candidate was reported to be excess power caused due noise appearing non-stationary between 60-200 Hz~\cite{GWTC1}. This candidate acts as an example of how $\pastrobcr$ may be utilised to not only to detect signals but also to eliminate terrestrial noise sources. 

\subsection{IAS Events}
Our analysis of the \av{double check that these fall in our search space!!} high-mass IAS events and candidates in O2 has resulted in three events (GW170202, GW170403, GW170425) with disfavourable $\pastrobcr<0.5$. Additionally, five events (GW170121, GW170304, GW170402, GW170727, GW170817A) and one candidate event (GWC170302) reported by IAS have $\pastrobcr>0.5$.

Event candidates GW170727 and GWC170302 are of particular interest, with \pastrobcr of 0.94 and 0.73. GW170727 was emitted from a black hole binary system with a source frame total mass $\approx 70\ \msun$. In addition to the high \pastrobcr reported by our study, \citet{IAS1} and \citet{pycbc_ogc_2} have also reported high \pastro values of 0.98 and 0.99. On the other hand, GWC170302 is a sub marginal-candidate reported by \cite{IAS1} with a \pastro of 0.45 appears to have a higher significance our analysis, resulting in a \pastrobcr of 0.73 .  

\av{GW170202's component black holes have has source masses (23, 16) below that of our prior space which may be why it has a low \pastrobcr}
...
\av{why do IAS2:GW170425 and IAS2:GW170402 have low \pastro?} 

\subsection{Novel Candidates}
Although no clear detections were made with the \bcr, a marginal-candidate GWC170222 has been discovered with a \pastrobcr of 0.53.  
