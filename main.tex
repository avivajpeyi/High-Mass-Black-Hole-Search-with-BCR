...
We assume that each detector's noise is Gaussian and stationary over the period being analysed \cite{ligo_psd}. In practice, we estimate the noise using a noise power spectral density \psd. The method of estimating the \psd, along with details on the implementation of our Bayesian inference pipeline is presented in the following section \av{(Not sure if the details on the PSD generation should be here or not...)}. Using the \psd, we can write 
\begin{equation}
Z^N_i =  \frac{1}{2\pi \sigma^2} \text{exp}\left(-\frac{1}{2} \frac{d_i}{\sigma^2} \right) \ ,
\end{equation}
for the data $d_i$ in each of the $i$ detectors in a network of $D$ detectors..  
