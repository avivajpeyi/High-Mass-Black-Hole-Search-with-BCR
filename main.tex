...
Although no clear detections were made with the \bcr, a marginal-candidate 170222 has been discovered with a \pastrobcr of $0.53$. This candidate has its similar masses when compared to those of GWTC-1. 
...
In this paper we demonstrate that the Bayesian Coherence Ratio~\cite{BCR1} can be used as a ranking statistic to provide a better measure of significance for gravitational wave candidates by re-analysing the significance of high-mass binary black hole triggers from O2. This method is takes a step towards building a unified Bayesian framework that provides a search-pipeline agnostic measure of significance, utilising the same level of physical information incorporated during parameter estimation. We focused our analysis to the high-mass regime as this region of the parameter space is plagued with a high number of short duration terrestrial artefacts that can mimic signals. Additionally, although we focused our analysis to the high-mass region, this method can be extended to include the entire set of gravitational wave candidates.
...
To improve our analysis, we may t

Future work:
\begin{itemize}
    \item increase nlive 
    \item atm just HL but we should do HVLK (more the detectors, more true, even if one detector is subthreshold)
    \item try to use more physical waveforms
    \item glitch model
    \item astrophysical priors rather than uniform
\end{itemize}
