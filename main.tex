...
\end{equation}
The numerator is the probability of obtaining a coherent signal, where $Z^S$ is the evidence for $\mathcal{H_S}$ and $\alpha$ is the prior odds for obtaining a signal $\alpha=P(\mathcal{H_S})/P(\mathcal{H_I})$. The denominator is the probability of obtaining an incoherent glitch or noise amongst the network of $D$ detectors, where $Z_{Gi}$ and $Z_{Ni}$ are the evidence for obtaining a glitch in detector $i$ and the evidence for obtaining just noise in detector $i$.
