...
\centering
\caption{
$\pastrobcr$ table for gravitational wave events and candidates in our search space with $\pastrobcr>0.2$, calculated using Hanford and Livingston observatory data. Displayed for comparison are significances of events taken from: GstLAL \pastroGwtcGstlal~\citep{GWTC1}, 
PyCBC \pastroGwtcPycbc~\citep{GWTC1}, 
IAS \pastroIas~\citep{IAS1, IAS2},  
\pastroPrat~\citep{bayesian_odds}, 
PyCBC `single-search' \pastroSing~\citep{pycbc_single_det}, 
PyCBC OGC-2 \pastroOgcTwo~\citep{pycbc_ogc_2} and
PyCBC OGC-3 \pastroOgcThree~\citep{pycbc_ogc_2}. 
The $\tc$ column contains the `GPS' coalescence-times of the gravitational wave events. 
The catalog column displays the first catalog reporting the event on each row (the catalogs labeled IAS-1 and IAS-2 correspond to the candidates published by \citealt{IAS1} and \citealt{IAS2}).
}
\label{tab:results}
